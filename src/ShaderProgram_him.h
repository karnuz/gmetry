...
public:
    ShaderProgram()
  {
    program = -1;
    enabled = false;
    shaders[0] = *(new ShaderInfo());
    shaders[1] = *(new ShaderInfo());
  }
   ...
   */
  void createProgram(OpenGLFunctions& gl,string vertShaderFile,string fragShaderFile)
  {
...
private:
  ...
  int createShaders(OpenGLFunctions& gl)
  {
    ifstream file;
	...
	if (!compiled)
	  {
	    ...
	    //		throw ShaderException(error_message);
	  }
